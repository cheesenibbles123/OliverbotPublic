...
exports.iterateOver = function iterateOver(rows, type){
	if (rows.length < 1){
		...
		for (let i = 0; i < rows.length; i++){
			returnString += `\`${rows[i].Name}\` : ${rows[i].Name}\n`;
		}
		return returnString;
	}
}
